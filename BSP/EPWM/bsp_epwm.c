...
{
    GPIO_SetupPinMux(EPWM_EN_PIN, GPIO_MUX_CPU1, 0x0);
    GPIO_SetupPinOptions(EPWM_EN_PIN, GPIO_OUTPUT, GPIO_PUSHPULL);

    GPIO_WritePin(EPWM_EN_PIN, EPWM_DISENABLE);//低电平有效。初始化为高电平
}


void bsp_Mos_State_Read(void)
{
    GPIO_SetupPinMux(EPWM_EN_PIN, GPIO_MUX_CPU1, 0x0);
    GPIO_SetupPinOptions(EPWM_EN_PIN, GPIO_OUTPUT, GPIO_PUSHPULL);
